  ...
  return (
    <div className="grid grid-cols-3 gap-3 mb-4">
      {(["1", "X", "2"] as const).map((label, i) => {
        const opens = [2.35, 3.40, 3.80];
        const deltas = [-0.25, -0.10, +0.15];
        return (
          <div key={label} className="border rounded-lg p-3 text-center bg-gray-50 border-gray-200">
            ...
            <p className="text-lg font-extrabold text-gray-600">{opens[i].toFixed(2)}</p>
            <p className={`text-xs font-semibold mt-1 ${deltas[i] < 0 ? "text-green-600" : "text-red-500"}`}>
              {deltas[i] > 0 ? "+" : ""}{deltas[i].toFixed(2)} {deltas[i] < 0 ? "↓" : "↑"}
            </p>
          </div>
        );
      })}
...
export function BlurPlaceholder({ section }: Props) {
  return (
    <div className="bg-white rounded-xl shadow p-5">
      <h2 className="text-sm font-bold text-gray-700 mb-4">{TITLES[section]}</h2>
